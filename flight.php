														...
														<?php
															foreach ($airports as $airport){
																if ($airport->country_id != 'id') {
																	$sel = '<option value="'.$airport->airport_code. '" ' .(strcmp($airport->airport_code, $s_origin)==0 ? 'selected' : ''). ' >'.trim($airport->airport_name).' ('.trim($airport->location_name).', '.trim($airport->country_name).')</option>"';
																} else {
																	$sel = '<option value="'.$airport->airport_code.'" ' .(strcmp($airport->airport_code, $s_origin)==0 ? 'selected' : ''). ' >'.trim($airport->airport_name).' ('.trim($airport->location_name).')</option>"';
																}
																echo $sel;
															}
														?>
